export class ResizeManager {
  ...
  private doResize(e: MouseEvent): void {
    ...
    const panel = this.currentHandle.closest(
      '.collections-panel, .request-panel, .json-input-panel'
    ) as HTMLElement;
    if (!panel) return;
    ...
    if (panelType === 'collections') {
      ...
      const requestPanel = layout?.querySelector(
        '.request-panel'
      ) as HTMLElement | null;
      ...
      const viewerPanel = panel.parentElement?.querySelector(
        '.json-viewer-panel'
      ) as HTMLElement | null;
  ...
  private rebalanceLayoutWidths(): void {
    ...
    const maxCollectionsByLayout =
      layoutWidth - requestMinWidth - responseMinWidth;
