...
export class TargetAdHocDataExtractor {
   ...
   */
  static getAuthConfig(container: HTMLElement): {
    type: 'none' | 'basic' | 'bearer' | 'apikey' | 'oauth2';
    data?: any;
  } {
    const authType = (
      container.querySelector('#target-auth-type') as HTMLSelectElement
    ).value;
   ...
   */
  static getBodyConfig(container: HTMLElement): {
    type: 'none' | 'json' | 'raw' | 'form-urlencoded';
    content: string;
  } {
    const bodyType = container.querySelector(
      'input[name="target-body-type"]:checked'
    ) as HTMLInputElement;
  ...
  static getTarget(container: HTMLElement): LoadTestTargetAdHoc {
    const method = (
      container.querySelector('#target-method') as HTMLSelectElement
    ).value as HttpMethod;
    const url = (container.querySelector('#target-url') as HTMLInputElement)
      .value;
